<script lang="ts">
  ...
  type ClassmateFriend = ClassmateRow;
  type WheelActivity = WheelActivityRow;

  let NEW_CLASSMATES: ClassmateFriend[] = [];
  let NPC_FREQ_TEMPLATES: NpcFreqTemplateRow[] = [];
  let WHEEL_ACTIVITIES: WheelActivity[] = [];

  const FREQ_ADVERB_PATTERN = /\b(always|usually|often|sometimes|seldom|never)\b/i;
  ...
  let phaseLabel = $state('Meet classmates · Spin · Ask');
  let friendsBarHtml = $state('');
  ...
  function updateHUD() {
    phaseLabel = `Round ${roundsDone} / ${PASS_ROUNDS} · Friends ${friendsCollected.length}/${PASS_ROUNDS}`;
  }
    ...
    const chips =
      friendsCollected.length > 0
        ? friendsCollected
            .map(
              (f) =>
                `<span class="friend-chip filter drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]" title="${f.label}">${f.emoji}</span>`,
            )
            .join('')
        : '<span class="text-emerald-500/50">—</span>';
    friendsBarHtml = `<span class="font-bold mr-1 text-emerald-400">Friends</span>${chips}<span class="ml-1 text-emerald-300/70">${friendsCollected.length}/${PASS_ROUNDS}</span>`;
  }

  function showFriendReward(friend: ClassmateFriend, onContinue: () => void) {
    phaseLabel = `New friend · ${friendsCollected.length}/${PASS_ROUNDS}`;
    render(`
  ...
  function buildWheelLabelText(tx: number, ty: number, rot: number, words: string[]): string {
  ...
  function buildWheelSVG(items: WheelActivity[]): string {
    ...
    const seg = 360 / n;
    const colors = ['#a5d6a7', '#81c784', '#66bb6a', '#4caf50', '#43a047', '#388e3c', '#2e7d32', '#1b5e20'];
    let paths = '';
    items.forEach((d, i) => {
  ...
  function validateHowOftenAnswer(text: string, act: WheelActivity, mode: 'adverb' | 'phrase'): string | null {
  ...
  function validateStudentHowLongQuestion(text: string, act: WheelActivity): string | null {
    ...
    if (act.be) {
      ...
      if (!/\bhow\s+long\s+does\s+it\s+take\b/.test(t) && !/\bhow\s+long\s+do\s+you\s+take\b/.test(t)) {
  ...
  function showSpinPhase(banner?: string) {
    phaseLabel = `Round ${roundsDone + 1} · Pick a topic`;
  ...
  function showHowOftenAnswerRound(mode: 'adverb' | 'phrase') {
    ...
    phaseLabel = isAdverb ? 'How often · answer (adverb)' : 'How often · answer (phrase)';
    ...
    render(`
      ...
      ${answerPanel(
        '',
        isAdverb ? 'This round: answer with a frequency word' : 'This round: answer with a frequency phrase',
        'ans',
        'btn',
      )}
    `);
  ...
  function showHowOftenStudentAsk() {
    const act = current!;
    phaseLabel = 'How often · Your turn to ask';
    render(`
  ...
  function showHowOftenNpcReply() {
    phaseLabel = 'How often · Classmate answers';
    render(`
      <div class="tech-panel mb-4">
        ${renderNpcBlock({ id: 'classmate', line: npcFreqAnswer })}
      </div>
      <div class="tech-panel text-center">
  ...
  function showHowLongNpcAsk() {
    const act = current!;
    phaseLabel = 'How long · Classmate asks';
    ...
    render(`
      <div class="tech-panel mb-4">
        ${renderNpcBlock({ id: 'classmate', line: howLongNpcQuestion(act) })}
      </div>
  ...
  function showHowLongStudentAsk() {
    const act = current!;
    phaseLabel = 'How long · Your turn to ask';
  ...
  function showHowLongNpcReply() {
    const act = current!;
    phaseLabel = 'How long · Classmate answers';
  ...
  function endGame(won: boolean) {
    stopAllTravelerAudio();
    if (won) {
      gameProgress.complete('traveler_quest_level2_complete');
      gameProgress.complete('traveler_quest_how_often_complete');
      gameProgress.complete('traveler_quest_how_long_complete');
    }
    const data = tracker.endGame(won ? 'completed' : 'abandoned', roundsDone, PASS_ROUNDS, PASS_ROUNDS);
  ...
  onMount(() => {
    ...
    void (async () => {
      NEW_CLASSMATES = await loadNewClassClassmates();
      NPC_FREQ_TEMPLATES = await loadNewClassNpcFreqTemplates();
      WHEEL_ACTIVITIES = await loadNewClassWheelActivities();
      updateFriendsBar();
      updateHUD();
      showIntro();
    })();
...
<svelte:head>
  <title>Level 2: New Class Icebreakers</title>
</svelte:head>

<div class="tq-root tq-level2 font-sans">
  <header class="bg-slate-800 shadow-lg p-4 flex justify-between items-center flex-wrap gap-2 border-b border-emerald-500/30 relative z-10">
    <div>
      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <!-- svelte-ignore a11y_click_events_have_key_events -->
      <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
      <h1 class="text-xl md:text-2xl font-bold text-emerald-400 cursor-pointer select-none" onclick={() => titleClickCount++}>
        🎒 New Class Icebreakers
      </h1>
      <p class="text-emerald-200/70 text-sm">{phaseLabel}</p>
    </div>
    <!-- eslint-disable-next-line svelte/no-at-html-tags -->
    <div class="friends-bar" aria-label="New friends collected">{@html friendsBarHtml}</div>
    <a href={WH_HUB_RETURN} onclick={() => stopAllTravelerAudio()} class="px-4 py-2 bg-slate-700 text-emerald-100 rounded-xl hover:bg-slate-600 border border-emerald-500/30 font-bold text-sm transition-colors">
      🏠 Back
    </a>
  </header>
