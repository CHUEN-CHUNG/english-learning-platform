<script lang="ts">
  ...
  type Destination = DestinationRow;
  type StatusCard = StatusCardRow;

  let DESTINATIONS: Destination[] = [];
  let STATUS_CARDS: StatusCard[] = [];
  ...
  // Reactive UI
  let progressLabel = $state('Destination 0 / 3');
  ...
  function updateHUD() {
    progressLabel = `Destination ${locationsDone} / ${PASS_LOCATIONS}`;
  }
  ...
  function getWheelDestinations(): Destination[] {
    const remaining = DESTINATIONS.filter((d) => !usedDestinations.includes(d.label));
  ...
  function transportRoundHint(mode: TransportMode): string {
    return mode === 'in_on' ? 'This round: use in or on' : 'This round: use by';
  }
  ...
  function validateTransport(answer: string, mode: TransportMode): { ok: boolean; feedback: string } {
    const a = normalize(answer).toLowerCase();

    if (mode === 'by') {
      if (/\bby\s+(a|an|the)\b/.test(a)) {
        return { ok: false, feedback: "Don't use a/the after by. Use e.g. by bus, by taxi." };
      }
      if (/\b(in the|in a|on the|on an)\b/.test(a) && !/\bby\b/.test(a)) {
        return { ok: false, feedback: 'This round use by + transport, e.g. by bus.' };
      }
      if (/\bby\s+(bus|taxi|train|plane|air|car|subway|metro|bike)\b/.test(a)) {
        return { ok: true, feedback: '' };
      }
      return { ok: false, feedback: 'Use by + transport, e.g. by bus / by taxi / by train.' };
    }

    if (/\bby\s+(bus|taxi|train|plane|car)\b/.test(a)) {
      return {
        ok: false,
        feedback: 'This round use in or on, not by bus. Say e.g. on the bus, in a taxi.',
      };
    }
    if (
    ...
    return { ok: false, feedback: "Use in or on, e.g. I'm on the bus, in a taxi, on foot." };
  }
  ...
  function validateStatus(answer: string, card: StatusCard): { ok: boolean; feedback: string } {
    const a = normalize(answer);
    if (card.expectedPatterns.some((p) => p.test(a)) || answerMatchesStatusLabel(a, card)) {
      return { ok: true, feedback: '' };
    }
    return { ok: false, feedback: `Your card is ${card.label}. ${card.hint}` };
  }
  ...
  function buildWheelSVG(wheelDests: Destination[]): string {
    ...
    const seg = 360 / n;
    const colors = ['#81d4fa', '#4fc3f7', '#29b6f6', '#03a9f4', '#039be5', '#0288d1', '#0277bd', '#01579b'];
    let paths = '';
    wheelDests.forEach((d, i) => {
      ...
      const short = display.length > 14 ? display.slice(0, 13) + '…' : display;
  ...
  function showIntro() {
    progressLabel = 'Departure briefing';
    render(`
  ...
  function showSpinPhase(banner?: string) {
    ...
    render(`
      ...
      <div class="tech-panel mb-4">
        ${renderNpcBlock({ id: 'guide', line: 'Spin the wheel — where shall we travel next?' })}
      </div>
  ...
  function showTransportPhase() {
    ...
    render(`
      <div class="tech-panel mb-4">
        ${renderNpcBlock({ id: 'guide', line: dest.question, subline: `Destination: ${dest.label}` })}
      </div>
    ...
    document.getElementById('btn-submit-transport')!.addEventListener('click', () => submitTransport(input, feedback));
  ...
  function showWhereAreYouPhase() {
    ...
    render(`
      <div class="tech-panel mb-4">
        ${renderNpcBlock({ id: 'guide', line: 'Where are you?', subline: whereAreYouTransportHint() })}
      </div>
  ...
  function showStatusPhase() {
    ...
    render(`
      <div class="tech-panel mb-4">
        ...
        <div class="tech-panel mb-4">
          ${renderNpcBlock({ id: 'guide', line: statusNpcLine(card, dest.label) })}
        </div>
        <label class="block text-sm font-bold text-sky-200/70 mb-2">Your answer (English):</label>
    ...
    document.getElementById('btn-submit-status')!.addEventListener('click', () => submitStatus(input, feedback));
    input.addEventListener('keydown', (e) => {
  ...
  function endGame(won: boolean) {
    stopAllTravelerAudio();
    if (won) {
      gameProgress.complete('traveler_quest_level1_complete');
    }
    const data = tracker.endGame(
  ...
  onMount(() => {
    ...
    void (async () => {
      DESTINATIONS = await loadTransportDestinations();
      STATUS_CARDS = await loadTransportStatusCards();
      showIntro();
    })();
...
<svelte:head>
  <title>Level 1: Departure &amp; Surprises</title>
</svelte:head>

<div class="tq-root font-sans">
  <header class="bg-slate-800 shadow-lg p-4 flex justify-between items-center flex-wrap gap-2 border-b border-sky-500/30 relative z-10">
    <div>
      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <!-- svelte-ignore a11y_click_events_have_key_events -->
      <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
      <h1 class="text-xl md:text-2xl font-bold text-sky-400 cursor-pointer select-none" onclick={() => titleClickCount++}>
        ✈️ Level 1: Departure &amp; Surprises
      </h1>
      <p class="text-sky-200/70 text-sm">{progressLabel}</p>
    </div>
    <div class="flex gap-3 items-center flex-wrap">
      <a href={WH_HUB_RETURN} onclick={() => stopAllTravelerAudio()} class="px-4 py-2 bg-slate-700 text-sky-100 rounded-xl hover:bg-slate-600 border border-sky-500/30 font-bold text-sm transition-colors">
        🏠 Back
      </a>
    </div>
  </header>

  <main bind:this={gameMain} class="flex-1 p-4 max-w-2xl mx-auto w-full space-y-4 relative z-10"></main>
</div>
