...
/** 判斷一筆紀錄是否屬於指定單元 */
export function recordMatchesUnit(record: any, unit: UnitRef | null): boolean {
  if (!unit) return true;
...
function buildQuizDetailHtml(record: any): string {
...
function buildReadingRecordHtml(
  record: any,
  ctx: { isTestUser: boolean; wordFails: Record<string, number>; studentWordFails: Record<string, number> }
): string {
  const { isTestUser, wordFails, studentWordFails } = ctx;
  let errorDetailsHtml = '';
  if (record.reviewData) {
    const errors = record.reviewData.filter((q: any) => !q.s1Correct || !q.s2Correct);
    if (errors.length > 0) {
  ...
  const gameTypeName =
    record.gameType === 'Matching'
      ? '🎮 連連看'
      : record.gameType === 'Reading'
        ? '📖 閱讀練習'
...
export function renderReadingDataPanel(options: { filterType?: string; unit?: UnitRef | null } = {}) {
  ...
  const wordFails: Record<string, number> = {};
  let html = '';

  for (const [user, data] of Object.entries<any>(allData)) {
    let history = data.history || [];
    const abandons = data.abandons || [];
    const profile = data.profile || {};

    if (filterType !== 'All') {
      history = history.filter((r: any) => r.gameType === filterType);
    }
    if (unit) {
      history = history.filter((r: any) => recordMatchesUnit(r, unit));
    }
    ...
    const studentWordFails: Record<string, number> = {};
    html += `
    ...
    const topStudentFails = Object.entries(studentWordFails)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
      ...
      html += `
            <div class="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg flex flex-wrap items-center gap-2 text-sm">
                ...
                ${topStudentFails
                  .map(
                    (f) =>
                      `<span class="bg-white text-red-600 px-2 py-0.5 rounded border border-red-200">${escapeHtml(f[0])} (${f[1]}次)</span>`
                  )
                  .join('')}
            </div>`;
    }
  ...
  if (elTopErrors) {
    const topFails = Object.entries(wordFails)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    elTopErrors.innerHTML = topFails.length
      ? topFails
          .map(
            (f) =>
              `<span class="bg-red-100 text-red-700 px-3 py-1 rounded-full font-bold border border-red-200 shadow-sm">${escapeHtml(f[0])} (${f[1]}次)</span>`
          )
          .join('')
  ...
  for (const [user, data] of Object.entries<any>(allData)) {
    ...
    if (history.length > 0) {
      ...
      history.forEach((record: any) => {
          ...
          statsHtml = record.stats
            .map((s: any, i: number) => {
              ...
              if (record.gameType === 'MultipleChoice') {
                ...
                specificMetrics = s.wrongSubmits
                  ? `錯誤送出 <span class="font-bold text-red-500">${s.wrongSubmits}</span> 次`
                  : '';
              }
        ...
        const gameTypeName =
          record.gameType === 'MultipleChoice'
            ? '🕳️ 選擇題'
            : record.gameType === 'Unscramble'
              ? '🚂 重組題'
              : record.gameType || '未知遊戲';
...
let panelState: { tab: 'reading' | 'grammar'; filterType: string; unit: UnitRef | null; unitTitle: string } = {
  tab: 'reading',
  filterType: 'All',
  unit: null,
  unitTitle: ''
...
function switchDataTab(tab: 'reading' | 'grammar') {
  panelState.tab = tab;
  ...
  const readingActive = 'border-blue-500 text-blue-600 whitespace-nowrap py-3 px-1 border-b-2 font-medium text-lg';
  const readingIdle =
  ...
  if (tab === 'reading') {
    ...
    if (contentReading) {
      ...
      (contentReading as HTMLElement).style.display = '';
    }
    if (contentGrammar) {
      contentGrammar.classList.add('hidden');
      (contentGrammar as HTMLElement).style.display = 'none';
    }
    applyReadingFilters();
  } else {
    ...
    if (contentGrammar) {
      ...
      (contentGrammar as HTMLElement).style.display = '';
    }
    if (contentReading) {
      contentReading.classList.add('hidden');
      (contentReading as HTMLElement).style.display = 'none';
    }
...
export function initTeacherDataPanel() {
  ...
  document.querySelectorAll('.rd-filter-btn').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      panelState.filterType = (e.target as HTMLElement).getAttribute('data-filter') || 'All';
      applyReadingFilters();
    });
  });
}
...
/** 從教師大廳單元卡片開啟數據面板 */
export function openTeacherDataPanel({ tab = 'reading', gameType = 'All', unit = null, unitTitle = '' }: PanelOptions = {}) {
